...
import com.edutech.api.form.account.*;
...
import com.edutech.api.storage.criteria.AccountCriteria;
...
import com.edutech.api.utils.ConvertUtils;
import com.edutech.api.utils.DateUtils;
import com.edutech.api.dto.ApiMessageDto;
import com.edutech.api.dto.ResponseListObj;
import com.edutech.api.dto.account.LoginDto;
...
import com.edutech.api.dto.account.ForgetPasswordDto;
...
import com.edutech.api.storage.repository.GroupRepository;
...
public class AccountController extends ABasicController{
    ...
    public ApiMessageDto<LoginDto> login(@Valid @RequestBody LoginForm loginForm, BindingResult bindingResult, HttpServletResponse response){
        ...
        if (account == null || !passwordEncoder.matches(loginForm.getPassword(), account.getPassword()) || !Objects.equals(account.getStatus() , EduTechConstant.STATUS_ACTIVE)) {
        ...
        qrJwt.setPemission(eduTechApiService.convertGroupToUri(account.getGroup().getPermissions())+appendStringRole);
    ...
    private String getAppendStringRole (Account account) {
        ...
        if(Objects.equals(account.getKind(), EduTechConstant.USER_KIND_ADMIN)) {
            appendStringRole = "/account/profile,/account/update_profile,/account/logout";
        }else  if(account.getKind().equals(EduTechConstant.USER_KIND_TEACHER)) {
            appendStringRole = "/teacher/my-profile,/teacher/update-profile,/teacher/my-subject,/account/logout,/class/client,/chapter/client,/lesson/client,/classnews/client";
        } else if (account.getKind().equals(EduTechConstant.USER_KIND_STUDENT)) {
            appendStringRole = "/student/my-profile,/student/update-profile,/account/logout,/class/client,/chapter/client,/lesson/client,/classnews/client";
    } else {
    ...
    public ApiMessageDto<String> createAdmin(@Valid @RequestBody CreateAccountAdminForm createAccountAdminForm, BindingResult bindingResult) {
        ...
        Integer groupKind = EduTechConstant.GROUP_KIND_EMPLOYEE;
        if(createAccountAdminForm.getKind().equals(EduTechConstant.USER_KIND_ADMIN)) {
            groupKind = EduTechConstant.GROUP_KIND_SUPER_ADMIN;
        }
    ...
    public ApiMessageDto<String> updateAdmin(@Valid @RequestBody UpdateAccountAdminForm updateAccountAdminForm, BindingResult bindingResult) {
        ...
        if (StringUtils.isNoneBlank(updateAccountAdminForm.getAvatarPath())) {
            if(!updateAccountAdminForm.getAvatarPath().equals(account.getAvatarPath())){
                //delete old image
                eduTechApiService.deleteFile(account.getAvatarPath());
            }
    ...
    public ApiMessageDto<ForgetPasswordDto> requestForgetPassword(@Valid @RequestBody RequestForgetPasswordForm forgetForm, BindingResult bindingResult){
        ...
        String otp = eduTechApiService.getOTPForgetPassword();
        ...
        //send email
        eduTechApiService.sendEmail(account.getEmail(),"OTP: "+otp, "Reset password",false);
    ...
    public ApiMessageDto<Long> forgetPassword(@Valid @RequestBody ForgetPasswordForm forgetForm, BindingResult bindingResult){
        ...
        if(account.getAttemptCode() >= EduTechConstant.MAX_ATTEMPT_FORGET_PWD){
        ...
        if(!account.getResetPwdCode().equals(forgetForm.getOtp()) ||
                (new Date().getTime() - account.getResetPwdTime().getTime() >= EduTechConstant.MAX_TIME_FORGET_PWD)){
            //tang so lan
    ...
    public ApiMessageDto<String> verify(@RequestBody @Valid VerifyForm verifyForm, BindingResult bindingResult){
        ...
        if(!account.getVerifyCode().equals(verifyForm.getOtp()) ||
           (new Date().getTime() - account.getVerifyTime().getTime() >= EduTechConstant.MAX_TIME_VERIFY_ACCOUNT)){
        ...
        account.setStatus(EduTechConstant.STATUS_ACTIVE);
        accountRepository.save(account);
